...
try/catch 특징
1. try/catch문 안에서, assert/revert/require을 통해 에러가 난다면 catch는 에러를 잡지 못하고,
                    개발자가 의도한 것으로 알고 정상적으로 프로그램이 끝남
try catch 문 밖에서 assert/revert/require을 통해 에러가 난다면 catch는 에러를 잡고, 에러를 핸들할 수 있다 .

2. 3가지 catch
catch Error(string memory reqson){...}: revert나 require를 통해 생성된 에러 용도
catch Panic(uint errorCode){...} : assert를 통해 생성된 에러가 날 때 이 catch에 잡힘
    errorCode는 솔리디티에서 정의된 Panic 에러 별로 나옴
catch(bytesmemoryLowLevelData)(...): 이 catch는 로우 레벨 에러를 잡음

3.어디서 쓰는가?
외부 컨트랙트 함수를 부를 때: 다른 스마트 컨트랙을 인스턴스화 하여서, try/catch문이 있는 스마트 컨트랙의 함수를 불러와서 사용
외부 스마트 컨트랙을 생성 할 때:다른 스마트 컨트랙을 인스턴스화 생성 할 때 사용
...
contract runner{
    ...
    event catchPanic(string _name, uint256 err);
    event catchLowLevelErr(string _name, bytes _err);

    math public mathInstacne = new math();

    function playTryCatch(uint256 _num1, uint256 _num2) public returns(uint256,bool){
        
        try mathInstacne.division(_num1, _num2) returns(uint256 value){
             return(value, true);
